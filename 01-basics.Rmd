...
Before we start, create a folder called _python-for-experiments_ (or with some other more suitable but meaningful name) in you user folder (this is where Anaconda's Jupyter Notebook expects to find them). Download the [exercise notebook](notebooks/Seminar 01. Basics.ipynb) and put it in this folder. Open Jupyter Notebook (see [Getting Started](#getting-started), if you forgot how you do that), navigate to the folder you created and open the downloaded notebook. You will need to switch between explanations here and the exercises in the notebook, so keep them both open.
...
As noted above, you can think about a variable as a labeled box you can store something in. That means that you can always "throw away" the old value and put something new. In case of variables, the "throwing away" part happens automatically, as the new value overwrites the old one. Check yourself, what will be final value of the variable in the code below? **Go to exercise #1 in the notebook.**
...
**Very important**: although assignments _look_ like mathematical equations, they are **not equations!** Assignments follow a **very important** rule that you must keep in mind when understanding assignments: the right side expression is evaluated _first_ until the final value is computed, then and only then the final value is assigned to the variable specified on the left side (put in the box). What this means is that you can use the same variable on _both_ sides! Let's take a look at this code:
```{python eval=FALSE}
x = 2
y = 5
x = x + y - 4
...
First, it takes _current_ values of all variables (`2` for `x` and `5` for `y`) and substitutes them into the expression. After that internal step, the expression looks like
```{python eval=FALSE}
x = 2 + 5 - 4
```
...
NUMBER_OF_LEGS = 2
```
I strongly recommend using constants and avoid hardcoding the values. First, if you have several identical values that mean different things (2 legs, 2 eyes, 2 ears, 2 vehicles per character, etc.), seeing a `2` in the code will not tell you what does this `2` mean (the legs? the ears? the score multiplier?). You can, of course, figure it out based on the code that uses this number but you could spare yourself that extra effort and use a constant instead. Then, you just read its name and the meaning of the value becomes apparent (and it is the meaning not the actual value that you are mostly interested in). Second, if you decide to _change_ that value (say, our main character is now a tripod), when using a constant means you have only one place to worry about, the rest of the code stays as is. If you hard-coded that number, you are in for an exciting (not really) but definitely long search-and-replace throughout the entire code.
...
You can explicitly convert from one type to another using special functions. For example, to turn a number or a logical value into a string, you simply write `str(<value>)`. In examples below, what would be the result (**go to exercise #6 in the notebook**)?
```{python eval=FALSE}
str(10 / 2)
str(2.5 + True)
str(True)
...
Similarly, you can convert to a logical/Boolean variable using `bool(<value>)` function. The rules are simple, for numeric values `0` is `False`, any other non-zero value is converted to `True`. For string, an empty string `''` is evaluated to `False` and non-empty string is converted to `True`. What would be the output in the examples below (**go to exercise #7 in the notebook**)?
...
**Go to exercise #9 and figure out what will be printed by the code below:**
```{python eval=FALSE}
dinner = "stake"
count = 4
desert = "cupcakes"
...
In case of real values you have two options: `%f` and `%g`. The latter uses scientific notation (e.g. `1e10` for `10000000000`) to make a representation more compact. To get a better feeling for the difference **do exercise #11**.
