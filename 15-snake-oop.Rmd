...
* `win` : you should pass with window variable to the constructor of an `Apple` class and store it as an attribute for later use when you (re)create the apple.
* `pos` : same as the field in the original dictionary but now as attribute
* `visuals` : same as the field in the original dictionary but now as attribute
...
Unfortunately, now you need to work through the code and functions you have implemented to make them work with these new classes. In most cases, the changes are straightforward. E.g., `create_snake_segment(win, ipos, square_size)` does not need the third argument anymore, as you can get it from `win` itself. Put breakpoints and use debug console, if you are unsure about the changes.

## Next time
Next time we will be programming _Moon Lander_ game using object-oriented programming approach from ground up!
