...
## An inedible apple
Let us add that highly desirable fruit: the apple! For the moment, we will represent it as a _differently_ colored square and snake won't be able to eat it. I assume that you used green color for the snake, so apples would yellow, red, or orange (but feel free to pick any color of your fancy). Just like a snake segment, an apple is characterized by its position on the grid and by its visuals, so you can use the same dictionary structure as for the snake's segment.

You need to write a new function `create_apple` that will be fairly similar to `create_snake_segment()` function you developed the last time. It should also return a dictionary with `pos` and `visuals` field. However, instead of taking a predefined grid location, it should generate one randomly within the grid and _not_ on the snake. The `is_inside_the_snake` function will help you to ensure the latter. Think about parameters that you need for this function. Remember to document it!

In the main loop, create a new variable `apple` and initialize it via `create_apple()` function. Think about where and when do you need to initialize it. Also, you need to add drawing the apple in the main loop. Again, think about where should you do it. 
...
Also, modify you code so that you start with a single segment snake.
...
## Eating yourself
Once our snake grows beyond four segments, it has an opportunity to bite itself^[Why at least five? Draw it on the grid and figure out whether it can do so even with four.]. For this, we need to check that, _after_ the snake moved, its head is _not_ ay the same location as the rest of its body. Create a new function `snake_bit_itself()` that takes the snake as a single parameter and returns `True` or `False` based on whether that is the case. The function is very similar but not identical to `is_inside_the_snake()` function you implemented earlier. What is the critical difference and why cannot you simply reuse it?

Once you implemented `snake_bit_itself()` function, you should check for that eventuaily after it moved and, if that is the case, the game should be over.
...
Let us give the player three attempts to achieve the top score. They have three lives, every time the snake dies, the game _but not the score_ resets: A single segment snake appears at the center and a new random apple appears elsewhere. Once the snake dies three times, the game is over. Think how you can implement this three repeatitions.

_Important change_, the score should be cumulative, so at the beinning of round two it should be equal to the final score of round one plus current length of the snake (1 at the very beginning). Think how you can achieve this.
...
Download [game-over-arcade.wav](sounds/game-over-arcade.wav)^[Downloaded from [freesound.org](https://freesound.org/) and created by [myfox14](https://freesound.org/people/myfox14/)] and [8-bit-game-over-sound.wav](sounds/8-bit-game-over-sound.wav)^[Also downloaded from [freesound.org](https://freesound.org/) and created by [EVRetro](https://freesound.org/people/EVRetro/)]. Use the former whenever the snake dies but use the latter when the player runs out of lives. In other words, the game should play a _different_ sound for the game over. You will need to use [Sound](https://www.psychopy.org/api/sound.html#sound) class from [sound](https://www.psychopy.org/api/sound.html) module of PsychoPy.
