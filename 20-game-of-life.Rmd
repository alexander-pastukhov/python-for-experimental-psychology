...
::: {.infobox .program}
Create `CellCultere` class and use it in the main code, drawing the cells. Put code in _exercise02.py_.
:::
...
So, what was your solution to deal with the border problem that I have described above? My solution is to simply pad the cell culture with an extra rows/columns of dead cells. Then, we count neighbours and modify state only of the _inner_ cells (so starting with cells in the second row and column) and keep the outer cells permanently dead. 
