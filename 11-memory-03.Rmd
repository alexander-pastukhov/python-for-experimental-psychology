...
Before we start, create a new folder _Memory03_ for exercise files. Copy _Images_ subfolder, as well as the `utilities.py` and _exercise11.py_ (rename it to _exercise01.py_, as we will use the latter as the staring point) from the previous seminar. Also, download [sounds](material/sound.zip) file and unzip it into a _Sounds_ subfolder in the seminar directory. 
